...
for (l in 1:100) {
  ...
  for (i in 1:length(taxa_all)) {
    ...
    for (k in 1:5) {
      if (k==1) {
        pollen_k <- subset(pollen_i, Age <= 0)[,c(which(colnames(pollen_i)==taxa_all[i]),6,7)]
        pollen_kplus <- subset(pollen_i, Age <= 2000 & Age > 0)[,c(which(colnames(pollen_i)==taxa_all[i]),6,7)]
      }
      if (k==2) {
        pollen_k <- subset(pollen_i, Age <= 2000 & Age > 0)[,c(which(colnames(pollen_i)==taxa_all[i]),6,7)]
        pollen_kplus <- subset(pollen_i, Age <= (k+1-2)*4000+2000 & Age > (k+1-3)*4000+2000)[,c(which(colnames(pollen_i)==taxa_all[i]),6,7)]
      }
      if (k > 2) {
        pollen_k <- subset(pollen_i, Age <= (k-2)*4000+2000 & Age > (k-3)*4000+2000)[,c(which(colnames(pollen_i)==taxa_all[i]),6,7)]
        pollen_kplus <- subset(pollen_i, Age <= (k+1-2)*4000+2000 & Age > (k+1-3)*4000+2000)[,c(which(colnames(pollen_i)==taxa_all[i]),6,7)]
      }
...
NC <- read.csv("./ClimateFidelity/NicheConservatismScore.csv")
Resilience <- read.csv("./ClimateFidelity/TNC.csv")
